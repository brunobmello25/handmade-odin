...
get_tile_chunk :: proc(tilemap: ^Tilemap, chunk_x, chunk_y, chunk_z: u32) -> ^Tilemap_Chunk {
	if chunk_x >= u32(tilemap.tile_chunk_count_x) ||
	   chunk_y >= u32(tilemap.tile_chunk_count_y) ||
	   chunk_z >= u32(tilemap.tile_chunk_count_z) {
		return nil
	}
	index :=
		chunk_z * u32(tilemap.tile_chunk_count_x) * u32(tilemap.tile_chunk_count_y) +
		chunk_y * u32(tilemap.tile_chunk_count_x) +
		chunk_x
	return &tilemap.tile_chunks[index]
}
...
get_tile_value :: proc(tilemap: ^Tilemap, abs_tile_x, abs_tile_y, abs_tile_z: u32) -> u32 {
	chunk_pos := get_chunk_position_for(tilemap, abs_tile_x, abs_tile_y, abs_tile_z)
...
set_tile_value :: proc(
	...
	assert(chunk_pos.tile_chunk_z < u32(tilemap.tile_chunk_count_z))
...
// Tile values: 1 = floor, 2 = wall, 3 = stair up, 4 = stair down
